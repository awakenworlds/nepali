...
        const pool = quizData
            .filter(c => c[key] !== correctAnswer && (c.sort === card.sort || c.sort2 === card.sort2)) // Included sort2 for better distractor generation
            .map(c => c[key]);
...
                    // We can prioritize card.sort for setting the dropdown, but the filter must check both.
